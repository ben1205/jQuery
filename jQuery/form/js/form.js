...
var $lastTr = $("tr:last");
$(".add").click(function(){
	$lastTr.before("<tr><td></td>" + $(this).parent().clone().html() + "<td></td></tr>")
});
...
//^对中括号内规则的取反
console.log(/[^\d]/g.test("123*1asdf23"));//等同于\D
...
console.log("123asdf214".match(/\d+?/));//?对+不会产生知足限制
...
//匹配手机号
console.log(/^1[3578]\d{9}$/.test("173456781011"));
...
console.log("332521196902060091".match(/^(?:(?:1[1-5]|2[1-3]|3[1-7]|4[1-6]|5[0-4]|6[1-5]|71|8[12])\d|8(86|5[23]))\d{3}(19\d{2}|20(0\d|1[0-6]))((0[13578]|1[02])(0[1-9]|[12]\d|3[01])|(0[469]|11)(0[1-9]|[12]\d|30)|02(0[1-9]|[12]\d))\d{3}[\dxX]$/));//?:加在小括号的前面可以使括号内的内容失效

//replace方法中第一个参数如果是正则，有存在分组，则第二个参数可以通过$n使用对应的分组，比如$1找到第一个小括号里匹配到的内容
console.log("asdf123asdf".replace(/(\w(\d))/, "$2$1$2"));

//正则表达式中使用\n可以获取到前面被小括号分组匹配到的内容
console.log("asdf123\1asdf".match(/(?:asdf)123\1/));

//匹配标签
console.log(/^<([a-z]+[a-z\d]*)[^>]*>.*<\/\1>$/.test("<h1>233</h1>"));
